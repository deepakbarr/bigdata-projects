package com.demo.util.geo_gen;

import com.demo.util.data.DataUtil;
...
public class  TrafficDataUtil extends DataUtil {
    ...
    private static final String OUTPUT_FILE = "/Users/dbarr/Garbage/data/supply_data_traffic2.csv";
    private static final String INPUT_FILE = "/Users/dbarr/Garbage/data/green_tripdata_2016-01.csv";
    public static final String ID_KEY = "driver_id";
    private static Map<String, List<String>> geoMap = new TreeMap();
...
//        new Writer().write(OUTPUT_FILE, transformed);
    ...
    public static List<String> transform(List<String> rows) throws JsonProcessingException {
        ...
        for (String row : rows) {
            ...
            record = new TrafficBaseRecord(ID_PREFIX + (INITIAL_ID + (++Id % totalDrivers)), formatter.format(new Date(current += 1000)), Double.parseDouble(tokens[6]), Double.parseDouble(tokens[5]),
                    Double.parseDouble(tokens[8]), Double.parseDouble(tokens[7]));
            try {
    ...
    private static void addToGeoMap(Double lat, Double lon) {
        String geoHash = GeoHash.encodeHash(lat, lon, 7);
