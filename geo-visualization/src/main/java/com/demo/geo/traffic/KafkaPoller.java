...
public class KafkaPoller {
    ...
    private final static String TOPIC = "traffic_output";
...
//        props.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earliest");
    ...
    public Map<String, TrafficRecord> runConsumer() throws InterruptedException, IOException {
        Map<String, TrafficRecord> records = new HashMap();
        final ConsumerRecords<String, String> consumerRecords = consumer.poll(500);
        if (consumerRecords.count() >= 0) {
            for (ConsumerRecord<String, String> record : consumerRecords) {
                String key = record.key();
                String json = record.value();
                TrafficRecord br = mapper.readValue(json, TrafficRecord.class);
                records.put(key, br);
            }
        }
